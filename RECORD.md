...
Append-only. One entry per unit of work, newest last.

## 1 - Project created (2026-08-30)

Charter and structure: four stores (world, corpus, voice,
training), each with its rules written at the door. License MIT.
The first world-document cut covers the national frame - the
United States of the early 1990s - with the game's Kentucky
layered in after. Published at
github.com/ellyj3rain/zomboid-speakeasy; CI lands with the first
runnable check.

## 2 - The horizon corrected; the first three documents drafted (2026-08-31)

The research plan was approved with three decisions: timeline,
daily life, and media as the floor; real names as history; primary
sources only. First drafts of all three exist, every claim dated,
cited, and confidence-marked. Then the plan's boundary was
corrected: the game's start date is a sandbox option (year list
anchored at 1993, any month, any day; shipped default July 9), a
...
C39 removed a requirement on the user, C40 removed a dependency
in the design: nothing of SAO now runs through another survivor
mod unless the player asks for it. Taking that mod's people over,
rewriting its per-survivor menu, and holding its prompts all sit
behind switches that default off, so a fresh world reaches into
another mod zero times. What stays always on calls none of their
code: SAO simply never treats their people as enemies. For this
project: a corpus gathered from play can assume every person in
the county is SAO's own, generated by SAO's own logic, with no
second authorship mixed in - which is what makes recorded speech
attributable to the world model we are building.
