...
def sealed(value: Any, schema_name: str) -> dict[str, Any]:
    Author.schema(value, schema_name)
    return Author.unseal(value, schema_name)
...
def validate_anchor(value: Any, catalogue: dict[str, Any],
                    catalogue_refs: list[str]) -> dict[str, Any]:
    ...
    require(value["task"] in TASKS, "anchor task must be understander or speaker")
...
def propose(catalogue: dict[str, Any], anchor: dict[str, Any], proposal_id: str,
            negative_labels: list[dict[str, str]]) -> dict[str, Any]:
    catalogue, refs = validate_catalogue(catalogue)
    anchor = validate_anchor(anchor, catalogue, refs)
...
def validate_proposal(value: Any) -> dict[str, Any]:
    ...
    anchor = validate_anchor(value["anchor"], catalogue, refs)
...
def validate_review(value: Any, proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    require(value["status"] in {"approved", "rejected"},
    ...
    fields(value["decision"], {"interactionId", "itemId", "value"},
           "review decision")
...
def admit(proposal: dict[str, Any], review: dict[str, Any], row_id: str) -> dict[str, Any]:
    proposal = validate_proposal(proposal)
    review = validate_review(review, proposal)
...
def validate_target(value: Any) -> dict[str, Any]:
    ...
    fields(value, {"schema", "schemaVersion", "rowId", "policy", "input", "anchor",
                   "labels", "provenance", "review", "standing", "contentSha256"},
           "retriever target")
    ...
    review = validate_review(value["review"], proposal)
...
def publish(path: Path, value: Any, inputs: list[Path]) -> None:
    ...
    try:
        if args.command == "propose":
            result = propose(Author.read(args.catalogue), Author.read(args.anchor),
                             args.proposal_id,
                             [parse_negative(item) for item in args.hard_negative])
        ...
        publish(args.out, result, inputs)
        return 0
